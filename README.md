...
- **Tides** — high/low tide predictions for today, toggle between Baltimore and Crisfield, MD (nearest official NOAA tide station to Pocomoke City). Free NOAA CO-OPS API, no key.
- **On This Day** — historical events for today's date, pulled from Wikipedia's free "On this day" API. Updates automatically once per day.
- **Scanner** — toggle between a Baltimore City feed and a Worcester County (Pocomoke-area) feed, with a "Listen Live" button that opens the feed on Broadcastify. Note: Broadcastify only allows direct audio embedding for a feed's own owner with a licensed domain key, so this links out rather than auto-playing inline — still one click to live audio.

## Customizing (scanner feeds)

Edit `SCANNER_FEEDS` in `server.js` to point at different Broadcastify feed IDs if you want different coverage areas.
